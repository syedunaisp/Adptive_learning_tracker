...
public class DatabaseSchema {
    ...
    private static void migrateSchema(Connection conn) throws SQLException {
        ...
        // if (oldVersion < 3) { createV3Tables(conn); setSchemaVersion(conn, 3); }
    }
    ...
    private static void seedV2Defaults(Connection conn) throws SQLException {
        try (Statement stmt = conn.createStatement()) {
            ...
            // --- Questionnaire Questions ---
            stmt.execute(
                    "INSERT OR IGNORE INTO questionnaire_questions (id, question_text, option_a, option_b, option_c, option_d) "
                            +
                            "VALUES (1, 'When studying a new topic, what do you do first?', " +
                            "'Read and re-read the material', 'Try to understand why it works', " +
                            "'Jump into practice problems', 'Look for diagrams or videos')");

            stmt.execute(
                    "INSERT OR IGNORE INTO questionnaire_questions (id, question_text, option_a, option_b, option_c, option_d) "
                            +
                            "VALUES (2, 'How do you best remember information for exams?', " +
                            "'Repeating it many times', 'Connecting it to things I already know', " +
                            "'Doing lots of practice questions', 'Drawing or visualizing it')");

            stmt.execute(
                    "INSERT OR IGNORE INTO questionnaire_questions (id, question_text, option_a, option_b, option_c, option_d) "
                            +
                            "VALUES (3, 'When you are stuck on a problem, what helps most?', " +
                            "'Reviewing my notes again', 'Thinking about the underlying concept', " +
                            "'Trying similar problems first', 'Sketching the problem out visually')");
        }
    }
